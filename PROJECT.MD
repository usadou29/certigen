...
- Micro-étapes :
  ...
  3. Implémenter l’endpoint d’extraction (`POST /api/ocr/attendance`) en partant d’un fichier `DOCX` plutôt que de Tesseract.  
     - 3.1 Repartir sur un parser DOCX (structure de tableau fiable) plutôt qu’un OCR lourd et peu fiable.  
     - 3.2 Écrire un service qui lit les tableaux Word (via `docx`, `mammoth` ou `y-docx`), détecte les colonnes nom/prénom et retourne `{ firstName, lastName }[]`.  
     - 3.3 Ajouter la route `POST /api/ocr/attendance` pour accepter `multipart/form-data` (fichier DOCX) et renvoyer directement les participants extraits.  
     - 3.4 Ajouter des tests unitaires/intégration (mock du parser DOCX) puis valider avec un fichier d’exemple pour garantir que la structure produit bien les colonnes attendues.  
